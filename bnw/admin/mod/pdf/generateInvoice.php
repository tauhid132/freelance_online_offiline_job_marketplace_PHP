...
$sql = "SELECT * FROM users WHERE printInvoice='1' ";
if($result = mysqli_query($conn, $sql)){
    if(mysqli_num_rows($result) > 0){
        ...
        while($row = mysqli_fetch_array($result)){
            ...
            $pdf->SetFont('Arial','',10);
            $pdf->Cell(60 ,6,$row['cus_name'],1,0,'C');
            $pdf->Cell(23 ,6,'',0,0);
            $pdf->SetFont('Arial','B',10);
            ...
            $pdf->Cell(30 ,6,'UserID/SubID',1,0,'C');
            $pdf->SetFont('Arial','',10);
            ...
            $pdf->SetFont('Arial','',10);
            $pdf->Cell(60 ,14,$row['conn_address'],1,0,'C');
            $pdf->Cell(23 ,6,'',0,0);
            $pdf->SetFont('Arial','B',10);
           ...
           $pdf->SetFont('Arial','',10);
           $pdf->Cell(45 ,6,$row['billing_type'],1,1,'C');
           ...
           $pdf->Cell(30 ,15,$row['monthly_bill'],1,0,'C');
           $pdf->Cell(20 ,15,'Nill',1,0,'C');
           $pdf->Cell(25 ,15,$row['monthly_bill'],1,1,'C');

           $dueCheck=$row['due']- $row['monthly_bill'];
           if($dueCheck>0){
              $pdf->Cell(10 ,15,'2',1,0,'C');
              $pdf->Cell(80 ,15,"Due Bill",1,0);
              $pdf->Cell(23 ,15,'',1,0,'C');
              $pdf->Cell(30 ,15,$row['due']-$row['monthly_bill'],1,0,'C');
              $pdf->Cell(20 ,15,'Nill',1,0,'C');
              $pdf->Cell(25 ,15,$row['due']-$row['monthly_bill'],1,1,'C');
          }

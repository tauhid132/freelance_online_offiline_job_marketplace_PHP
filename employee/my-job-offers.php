								...
								<div class="table-responsive">
									<table id="example1" class="table table-bordered table-striped">
										...
										<tbody>
											...
											$email = $_SESSION['email'];
												$sql = "SELECT * , job_offers.status as jStatus, job_offers.id as jid FROM job_offers join service_portfolio on job_offers.serviceId = service_portfolio.id join employer on job_offers.offeredBy = employer.emailAddress WHERE service_portfolio.employee_email = '$email'";
											if($result = mysqli_query($conn, $sql)){
												if(mysqli_num_rows($result) > 0){
													...
													while($row = mysqli_fetch_array($result)){
														...
														if($row['jStatus'] == 0){
															echo '<td>
														<a href="action/accept-job-proposal-action.php?job-id='.$row['jid'].'">
														<button class="btn btn-sm btn-primary"> Accept</button>
														</a>
														<a href="action/decline-job-proposal.php?job-id='.$row['jid'].'">

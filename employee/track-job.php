...
$job_id = $_GET['job-id'];
$query2=mysqli_query($conn,"SELECT * FROM hire_employee join employer on hire_employee.employerEmail = employer.emailAddress join service_portfolio on hire_employee.servicePortfolioId = service_portfolio.id left join job_payments on job_payments.jobId = $job_id where hire_employee.id = '$job_id'");

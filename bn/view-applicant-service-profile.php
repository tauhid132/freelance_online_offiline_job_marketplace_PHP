...
$proposal_id = $_GET['proposal-id'];

$queryforstatus=mysqli_query($conn,"SELECT * FROM apply_job  WHERE id = '$proposal_id';");
$resultForStatus=mysqli_fetch_array($queryforstatus);
$currentStatus = $resultForStatus['status'];
?>
<!DOCTYPE html>
<html lang="en">
		...
		<div class="container">
			<div class="row">
					...
					<div class="job-item ptrl_2 mt-10" style="padding-bottom: 30px; ">
						...
						<div class="job_dts" style="border-bottom: 1px solid #e5e5e5; padding-bottom: 30px;">
							...
							<ul>
								<li>
									<div class="job_dt_1">
										<h6>Service Working Type :</h6>
										<span><?php echo $result2['service_working_type']; ?></span>
									</div>
								</li>
								<li>
									<div class="job_dt_1">
										<h6>Available Areas :</h6>
				...
				?>
					...
					<form id="hire-form" method="post" action="action/accept-applicant-proposal-action.php">
						...
						<button class="apply_job_rt3" type="submit" data-toggle="modal" data-target="#applyjobModal"><i class="fa fa-check"></i> Accept & Hire</button>
					</form>
					<a href="action/decline-applicant-application.php?id=<?php echo $proposal_id?>"><button class="apply_job_rt2" type="submit" ><i class="fa fa-times-circle"></i> Decline</button></a>
				<?php
				}
				?>
					
					<a href="employer/view-messages.php?email=<?php echo $result2['emailAddress'] ?>"><button class="apply_job_rt" type="submit" data-toggle="modal" data-target="#applyjobModal"><i class="fa fa-envelope-o"></i> Message</button></a>
				...
				<div class="col-lg-9 col-md-8 mt-20">
					...
					<div class="job-item ptrl_2 mt-20 ">
						<div class="view_chart_header">
							...
							<h4 class="rating_header" style="float:right; font-size: 25px;"><i class="fas fa-star"></i> 5.0 / 5.0</h4>
						</div>
						
						<ul class="all_applied_jobs jobs_bookmarks">
							<?php
							$totalRating = 0;
							$sql = "SELECT * FROM hire_employee Join employer on hire_employee.employerEmail = employer.emailAddress WHERE servicePortfolioId = '$portfolio_id' and jobRating!=0";
							if($result = mysqli_query($conn, $sql)){
								if(mysqli_num_rows($result) > 0){
									while($row = mysqli_fetch_array($result)){
										 $totalRating = $totalRating + $row['jobRating'];
